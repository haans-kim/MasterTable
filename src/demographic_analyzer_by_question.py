...
"""
CJ 컬처서베이 인구통계학적 변수별 분석 - 문항별 페이지 구성
- 각 문항별로 시트 생성
- 한 시트에 모든 인구통계 그룹의 결과 포함
"""
...
class DemographicAnalyzerByQuestion:
    def __init__(self):
        self.okt = Okt()
    ...
    def analyze_question_for_group(self, data, q_name, col_name):
        """특정 그룹의 특정 문항 분석"""
        # 유효 응답 추출
    ...
    def run_analysis(self):
        """전체 분석 실행 - 문항별 페이지 구성"""
        print("\n" + "="*70)
        print("CJ 컬처서베이 인구통계학적 변수별 분석 - 문항별 페이지")
        print("="*70)
        ...
        output_file = f'CJ_voice_on_demographic_by_question_{timestamp}.xlsx'
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for q_name, col_name in self.question_map.items():
                ...
                # 각 인구통계 그룹별 분석
                for demo_var, demo_value in self.demographic_groups:
                    ...
                    # 문항 분석
                    topic_results, valid_count = self.analyze_question_for_group(
                        demo_data, q_name, col_name
                    )

                    print(f"유효응답: {valid_count}개, 토픽: {len(topic_results)}개")

                    # 그룹명 및 응답자 수
                    group_title = pd.DataFrame([[f'■ {group_name} (전체: {total_count:,}명, 유효응답: {valid_count:,}개)']])
                    group_title.to_excel(writer, sheet_name=sheet_name,
                                       startrow=current_row, index=False, header=False)
                    current_row += 1
...
def main():
    analyzer = DemographicAnalyzerByQuestion()
